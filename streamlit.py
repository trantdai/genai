...
if questions:
    ...
    if questions.lower() == "createpr":
        with st.chat_message('assitant'):
            answer = "Sure let me create a PR for you"
            st.markdown(answer)
        ...
        gh.git.create_ref(ref=f"refs/head/{branch_name}", sha=sha)
        ...
        if len(response['citations'][0]['retrievedReferences']) != 0:
            if "pull_request.md" in response['citations'][0]['retrievedReferences'][0]['location']['s3Location']['uri']:
                with st.chat_message('assistant'):
                    st.markdown("If you would like me to create a Pull Request then please say `createpr`")
